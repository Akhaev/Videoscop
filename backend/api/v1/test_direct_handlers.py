...
def test_full_flow():
    ...
    invalid_user_data = user.copy()
    invalid_user_data["email"] = "invalid_email"
    create_user_invalid_response = requests.post(f"{BASE_URL}/users", json=invalid_user_data)
    ...
    invalid_update_data = updated_user.copy()
    invalid_update_data["email"] = "invalid_email"
    update_user_invalid_response = requests.put(f"{BASE_URL}/users/me", json={**invalid_update_data, 'current_password': user['password']}, headers=headers)
    log_response("Редактирование с некорректными данными", update_user_invalid_response, {**invalid_update_data, 'current_password': user['password']})
