...
class User(Base):
    ...
    videos: Mapped[list["Video"]] = relationship(back_populates="author", passive_deletes='all')
...
class Video(Base):
    ...
    author_uuid = mapped_column(UUID(as_uuid=True), sa.ForeignKey('users.uuid', ondelete='CASCADE'), nullable=False)  # Внешний ключ на уровне строки
...
class HeatMap(Base):
    ...
    uuid = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    video_uuid = mapped_column(UUID(as_uuid=True), sa.ForeignKey('videos.uuid', ondelete='CASCADE'), nullable=False)  # Внешний ключ на уровне строки
    video: Mapped["Video"] = relationship(back_populates="heat_map", cascade='delete')
